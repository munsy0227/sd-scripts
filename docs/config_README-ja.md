...
* `bucket_no_upscale`
    * 元画像の拡大を防ぎます。最初に通常の定義済みbucketを選択し、縮小だけでそのbucketを満たせる画像には、このオプションが無効な場合と同じresizeとcropを適用します。
    * 選択されたbucketに合わせるために拡大が必要な場合のみ、元画像のサイズからより小さなbucketを作成します。各辺は `bucket_reso_steps` の倍数に切り捨てられ、余った端はpaddingではなくcropされます。
